...
void DMAInitialize(void)
{
    ...
    */
    ...
    NOP;       /* one NOP required after HARDRESET
...
* DMACH1AddrConfig - 
*/
void DMACH1AddrConfig(volatile uint16_t *DMA_Dest,volatile uint16_t *DMA_Source)
{
    EALLOW;
    ...
    */
    DmaRegs.CH1.SRC_BEG_ADDR_SHADOW = (uint32_t)DMA_Source;
    
    DmaRegs.CH1.SRC_ADDR_SHADOW =     (uint32_t)DMA_Source;
    ...
    */
    DmaRegs.CH1.DST_BEG_ADDR_SHADOW = (uint32_t)DMA_Dest;
    
    DmaRegs.CH1.DST_ADDR_SHADOW =     (uint32_t)DMA_Dest;
...
* DMACH1BurstConfig - 
*/
void DMACH1BurstConfig(uint16_t bsize, int16_t srcbstep, int16_t desbstep)
{
	EALLOW;
...
*/
void DMACH1TransferConfig(uint16_t tsize, int16_t srctstep, int16_t deststep)
{
	EALLOW;
...
* DMACH1WrapConfig -
*/
void DMACH1WrapConfig(uint16_t srcwsize, int16_t srcwstep, uint16_t deswsize, int16_t 
                 deswstep)
{
	EALLOW;
...
* DMACH1ModeConfig - 
*/
void DMACH1ModeConfig(uint16_t persel, uint16_t perinte, uint16_t oneshot, uint16_t cont, 
                 uint16_t synce, uint16_t syncsel, uint16_t ovrinte, uint16_t datasize,
                 uint16_t chintmode, uint16_t chinte)
{
    EALLOW;
    ...
    */
    DmaRegs.CH1.CONTROL.bit.PERINTCLR = 1U;/* Clear any spurious interrupt flags*/
    DmaRegs.CH1.CONTROL.bit.SYNCCLR = 1U;  /* Clear any spurious sync flags*/
    DmaRegs.CH1.CONTROL.bit.ERRCLR = 1U;  /* Clear any spurious sync error flags*/

    /*
    * Initialize PIE vector for CPU interrupt:
    */
    PieCtrlRegs.PIEIER7.bit.INTx1 = 1U;   /* Enable DMA CH1 interrupt in PIE*/
...
* DMACH2AddrConfig - 
*/
void DMACH2AddrConfig(volatile uint16_t *DMA_Dest,volatile uint16_t *DMA_Source)
{
    EALLOW;
    ...
    * Point to beginning of source buffer
    */
    DmaRegs.CH2.SRC_BEG_ADDR_SHADOW = (uint32_t)DMA_Source;
    DmaRegs.CH2.SRC_ADDR_SHADOW =     (uint32_t)DMA_Source;
    ...
    * Point to beginning of destination buffer
    */
    DmaRegs.CH2.DST_BEG_ADDR_SHADOW = (uint32_t)DMA_Dest;
    DmaRegs.CH2.DST_ADDR_SHADOW =     (uint32_t)DMA_Dest;
...
* DMACH2BurstConfig -
*/
void DMACH2BurstConfig(uint16_t bsize, int16_t srcbstep, int16_t desbstep)
{
	EALLOW;
...
*/
void DMACH2TransferConfig(uint16_t tsize, int16_t srctstep, int16_t deststep)
{
	EALLOW;
...
* DMACH2WrapConfig - 
*/
void DMACH2WrapConfig(uint16_t srcwsize, int16_t srcwstep, uint16_t deswsize, 
                 int16_t deswstep)
{
	EALLOW;
...
* DMACH2ModeConfig - 
*/
void DMACH2ModeConfig(uint16_t persel, uint16_t perinte, uint16_t oneshot, uint16_t cont, 
                 uint16_t synce, uint16_t syncsel, uint16_t ovrinte, uint16_t datasize,
                 uint16_t chintmode, uint16_t chinte)
{
    EALLOW;
    ...
    * Initialize PIE vector for CPU interrupt
    */
    PieCtrlRegs.PIEIER7.bit.INTx2 = 1U;      /* Enable DMA CH2 interrupt in PIE*/
...
* DMACH3AddrConfig - 
*/
void DMACH3AddrConfig(volatile uint16_t *DMA_Dest,volatile uint16_t *DMA_Source)
{
    EALLOW;
    ...
    * Point to beginning of source buffer
    */
    DmaRegs.CH3.SRC_BEG_ADDR_SHADOW = (uint32_t)DMA_Source;
    DmaRegs.CH3.SRC_ADDR_SHADOW =     (uint32_t)DMA_Source;
    ...
    * Point to beginning of destination buffer
    */
    DmaRegs.CH3.DST_BEG_ADDR_SHADOW = (uint32_t)DMA_Dest;
    
    DmaRegs.CH3.DST_ADDR_SHADOW =     (uint32_t)DMA_Dest;
...
* DMACH3BurstConfig - 
*/
void DMACH3BurstConfig(uint16_t bsize, int16_t srcbstep, int16_t desbstep)
{
	EALLOW;
...
*/
void DMACH3TransferConfig(uint16_t tsize, int16_t srctstep, int16_t deststep)
{
	EALLOW;
...
* DMACH3WrapConfig - 
*/
void DMACH3WrapConfig(uint16_t srcwsize, int16_t srcwstep, uint16_t deswsize, 
                 int16_t deswstep)
{
	EALLOW;
...
* DMACH3ModeConfig - 
*/
void DMACH3ModeConfig(uint16_t persel, uint16_t perinte, uint16_t oneshot, uint16_t cont, 
                 uint16_t synce, uint16_t syncsel, uint16_t ovrinte, uint16_t datasize,
                 uint16_t chintmode, uint16_t chinte)
{
	EALLOW;
    ...
    * Clear any spurious interrupt flags
    */
	DmaRegs.CH3.CONTROL.bit.PERINTCLR = 1U;
    
	DmaRegs.CH3.CONTROL.bit.SYNCCLR = 1U;   	/* Clear any spurious sync flags*/
	
    /*
    * Clear any spurious sync error flags
    */
    DmaRegs.CH3.CONTROL.bit.ERRCLR = 1U;

    /*
	* Initialize PIE vector for CPU interrupt:
    */
	PieCtrlRegs.PIEIER7.bit.INTx3 = 1U;     /* Enable DMA CH3 interrupt in PIE*/
...
* DMACH4AddrConfig - 
*/
void DMACH4AddrConfig(volatile uint16_t *DMA_Dest,volatile uint16_t *DMA_Source)
{
    EALLOW;
    ...
    * Point to beginning of source buffer
    */
    DmaRegs.CH4.SRC_BEG_ADDR_SHADOW = (uint32_t)DMA_Source;
    
    DmaRegs.CH4.SRC_ADDR_SHADOW =     (uint32_t)DMA_Source;
    ...
    * Point to beginning of destination buffer
    */
    DmaRegs.CH4.DST_BEG_ADDR_SHADOW = (uint32_t)DMA_Dest;
    
    DmaRegs.CH4.DST_ADDR_SHADOW =     (uint32_t)DMA_Dest;
...
* DMACH4BurstConfig - 
*/
void DMACH4BurstConfig(uint16_t bsize, int16_t srcbstep, int16_t desbstep)
{
	EALLOW;
...
*/
void DMACH4TransferConfig(uint16_t tsize, int16_t srctstep, int16_t deststep)
{
	EALLOW;
...
* DMACH4WrapConfig - 
*/
void DMACH4WrapConfig(uint16_t srcwsize, int16_t srcwstep, uint16_t deswsize, 
                 int16_t deswstep)
{
	EALLOW;
...
* DMACH4ModeConfig - 
*/
void DMACH4ModeConfig(uint16_t persel, uint16_t perinte, uint16_t oneshot, uint16_t cont, 
                 uint16_t synce, uint16_t syncsel, uint16_t ovrinte, uint16_t datasize,
                 uint16_t chintmode, uint16_t chinte)
{
    EALLOW;
    ...
    * Clear any spurious interrupt flags
    */
	DmaRegs.CH4.CONTROL.bit.PERINTCLR = 1U;
    
	DmaRegs.CH4.CONTROL.bit.SYNCCLR = 1U;   	/* Clear any spurious sync flags*/
	
    /*
    * Clear any spurious sync error flags
    */
    DmaRegs.CH4.CONTROL.bit.ERRCLR = 1U;

    /*
	* Initialize PIE vector for CPU interrupt:
    */
	PieCtrlRegs.PIEIER7.bit.INTx4 = 1U;     /* Enable DMA CH4 interrupt in PIE*/
...
* DMACH5AddrConfig - 
*/
void DMACH5AddrConfig(volatile uint16_t *DMA_Dest,volatile uint16_t *DMA_Source)
{
	EALLOW;
    ...
    * Point to beginning of source buffer
    */
    DmaRegs.CH5.SRC_BEG_ADDR_SHADOW = (uint32_t)DMA_Source;
    
	DmaRegs.CH5.SRC_ADDR_SHADOW =     (uint32_t)DMA_Source;
    ...
    * Point to beginning of destination buffer
    */
	DmaRegs.CH5.DST_BEG_ADDR_SHADOW = (uint32_t)DMA_Dest;
    
	DmaRegs.CH5.DST_ADDR_SHADOW =     (uint32_t)DMA_Dest;
...
* DMACH5BurstConfig - 
*/
void DMACH5BurstConfig(uint16_t bsize, int16_t srcbstep, int16_t desbstep)
{
	EALLOW;
...
*/
void DMACH5TransferConfig(uint16_t tsize, int16_t srctstep, int16_t deststep)
{
	EALLOW;
...
* DMACH5WrapConfig - 
*/
void DMACH5WrapConfig(uint16_t srcwsize, int16_t srcwstep, uint16_t deswsize, 
                 int16_t deswstep)
{
	EALLOW;
...
* DMACH5ModeConfig - 
*/
void DMACH5ModeConfig(uint16_t persel, uint16_t perinte, uint16_t oneshot, uint16_t cont, 
                 uint16_t synce, uint16_t syncsel, uint16_t ovrinte, uint16_t datasize,
                 uint16_t chintmode, uint16_t chinte)
{
	EALLOW;
    ...
    * Clear any spurious interrupt flags
    */
	DmaRegs.CH5.CONTROL.bit.PERINTCLR = 1U;
    
	DmaRegs.CH5.CONTROL.bit.SYNCCLR = 1U;   	/* Clear any spurious sync flags*/
	
    /*
    * Clear any spurious sync error flags
    */
    DmaRegs.CH5.CONTROL.bit.ERRCLR = 1U;

    /*
	* Initialize PIE vector for CPU interrupt:
    */
	PieCtrlRegs.PIEIER7.bit.INTx5 = 1U;     /* Enable DMA CH5 interrupt in PIE*/
...
* DMACH6AddrConfig - 
*/
void DMACH6AddrConfig(volatile uint16_t *DMA_Dest,volatile uint16_t *DMA_Source)
{
	EALLOW;
    ...
    * Point to beginning of source buffer
    */
    DmaRegs.CH6.SRC_BEG_ADDR_SHADOW = (uint32_t)DMA_Source;
    
	DmaRegs.CH6.SRC_ADDR_SHADOW =     (uint32_t)DMA_Source;
    ...
    * Point to beginning of destination buffer
    */
    DmaRegs.CH6.DST_BEG_ADDR_SHADOW = (uint32_t)DMA_Dest;
    
	DmaRegs.CH6.DST_ADDR_SHADOW =     (uint32_t)DMA_Dest;
...
* DMACH6BurstConfig - 
*/
void DMACH6BurstConfig(uint16_t bsize, int16_t srcbstep, int16_t desbstep)
{
	EALLOW;
...
*/
void DMACH6TransferConfig(uint16_t tsize, int16_t srctstep, int16_t deststep)
{
	EALLOW;
...
* DMACH6WrapConfig - 
*/
void DMACH6WrapConfig(uint16_t srcwsize, int16_t srcwstep, uint16_t deswsize, 
                 int16_t deswstep)
{
	EALLOW;
...
* DMACH6ModeConfig - 
*/
void DMACH6ModeConfig(uint16_t persel, uint16_t perinte, uint16_t oneshot, uint16_t cont, 
                 uint16_t synce, uint16_t syncsel, uint16_t ovrinte, uint16_t datasize,
                 uint16_t chintmode, uint16_t chinte)
{
	EALLOW;
    ...
    * Clear any spurious interrupt flags
    */
	DmaRegs.CH6.CONTROL.bit.PERINTCLR = 1U;
    
	DmaRegs.CH6.CONTROL.bit.SYNCCLR = 1U;   	/* Clear any spurious sync flags*/
	
    /*
    * Clear any spurious sync error flags
    */
    DmaRegs.CH6.CONTROL.bit.ERRCLR = 1U;

    /*
	* Initialize PIE vector for CPU interrupt:
    */
	PieCtrlRegs.PIEIER7.bit.INTx6 = 1U;     /* Enable DMA CH6 interrupt in PIE*/
...
{
    EALLOW;
    DmaRegs.CH6.CONTROL.bit.RUN = 1U;
    EDIS;
}
